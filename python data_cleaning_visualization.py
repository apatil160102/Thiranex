...
A Python project that takes a raw, messy movies dataset and cleans, processes, and visualizes it to surface key insights.

## What it does

- **Handles missing values** — using median imputation, with genre-aware medians for budget/revenue
- **Handles duplicates** — detects and removes exact duplicate rows
- **Handles outliers** — removes impossible values (e.g. ratings outside 1-10) and caps statistical outliers using the IQR method
- **Standardizes inconsistent text formatting** (e.g. `"ACTION"`, `" Action "`, `"action"` → `"Action"`)
- **Visualizes findings** using Matplotlib & Seaborn: rating distributions, budget vs revenue, correlation heatmap, genre-level comparisons
- **Builds a summary dashboard** combining the key charts into a single image
...
This will:
1. Generate a synthetic raw dataset (`raw_movies.csv`) with realistic data-quality issues
2. Clean it and save the result (`cleaned_movies.csv`)
3. Save chart images: `distributions.png`, `relationships.png`, `genre_analysis.png`, `dashboard.png`

## Using a real dataset instead

Replace the call to `generate_raw_data()` in `main()` with:

```python
raw_df = pd.read_csv("your_dataset.csv")
```

and adjust column names in the cleaning functions to match your data.

## Sample findings

- Comedy had the highest average rating and average profit in this dataset
- ~6% missingness was found across key numeric columns and was imputed
- A handful of extreme budget outliers were capped rather than dropped, to preserve other data for those rows
